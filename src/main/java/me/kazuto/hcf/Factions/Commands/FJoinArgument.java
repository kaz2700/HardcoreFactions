  ...
  public boolean onCommand(
    ...
    if (!(commandSender instanceof Player player)) {
      commandSender.sendMessage(
          String.format(
              "%s%sYou are the console so you can't join a faction!",
    ...
    if (FactionManager.getInstance().getFactionFromName(strings[1]) != null) {
      ...
      faction =
          FactionManager.getInstance()
              .getFactionFromPlayer(
                  FactionPlayerManager.getInstance().getPlayerFromName(strings[1]));
    } else {
      player.sendMessage(
          String.format("%s%sFaction not found.", Config.ERROR_COLOR, Config.ERROR_PREFIX));
      return false;
    }
    ...
    if (!faction.getInvitedPlayers().contains(joiningPlayer) && !faction.isOpen()) {
      player.sendMessage(
          String.format(
              "%s%sYou are not invited to the faction.", Config.ERROR_COLOR, Config.ERROR_PREFIX));
      return false;
    }
