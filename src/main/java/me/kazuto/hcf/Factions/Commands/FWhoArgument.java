    ...
    switch (strings.length) {
      case 1: // /F who
        if (!(commandSender instanceof Player)) {
          commandSender.sendMessage(
              String.format(
                  "%s%sYou are the console so you dont have a faction",
        ...
        if (FactionManager.getInstance().getFactionFromPlayer(factionPlayer) == null) {
          player.sendMessage(
              String.format(
                  "%s%sYou are not in a faction.", Config.ERROR_COLOR, Config.ERROR_PREFIX));
          return false;
        }

        PlayerFaction playerFaction =
            FactionManager.getInstance().getFactionFromPlayer(factionPlayer);
      ...
      case 2:
        ...
        for (FactionPlayer factionPlayer1 : FactionPlayerManager.getInstance().getPlayers()) {
          if (factionPlayer1.getName().equalsIgnoreCase(strings[1])
              && factionPlayer1.hasAFaction()) {
            output.append(
                FactionManager.getInstance().getFactionFromPlayer(factionPlayer1).getInfo());
          }
        }
      ...
      default:
        commandSender.sendMessage(
            String.format("%s%s/f who <name>.", Config.ERROR_COLOR, Config.ERROR_PREFIX));
        return false;
    }
    return true;
  }
