...
public class FUninviteArgument extends CommandArgument {
  ...
  public boolean onCommand(
    ...
    FactionPlayer uninvitedFactionPlayer =
    ...
    if (!faction.getInvitedPlayers().contains(uninvitedFactionPlayer)) {
      player.sendMessage(
          String.format(
              "%s%s%s is not invited to the faction.",
              Config.ERROR_COLOR, Config.ERROR_PREFIX, uninvitedFactionPlayer.getName()));
      return false;
    }

    if (uninvitedFactionPlayer.isOnline())
      uninvitedFactionPlayer
          .getOfflinePlayer()
          .getPlayer()
          .sendMessage(
              String.format(
                  "%s%s has revoked your invitation to join %s.",
