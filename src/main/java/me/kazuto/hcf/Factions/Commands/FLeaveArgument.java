...
public class FLeaveArgument extends CommandArgument {
    ...
    public boolean onCommand(CommandSender commandSender, Command command, String s, String[] strings) {
        if(!(commandSender instanceof Player player)) {
            commandSender.sendMessage(String.format("%s%sYou are the console so you can't leave a faction!", Config.ERROR_COLOR, Config.ERROR_PREFIX));
            return false;
        }
        ...
        FactionPlayer leavingPlayer = FactionPlayerManager.getInstance().getPlayerFromUUID(player.getUniqueId());

        if(!leavingPlayer.hasAFaction()) {
            player.sendMessage(String.format("%s%sYou must have a faction before leaving.", Config.ERROR_COLOR, Config.ERROR_PREFIX));
            return false;
        }
        ...
        if (faction.getLeader() == leavingPlayer) {
            player.sendMessage(String.format("%s%sYou are the leader of the faction. Use: /f disband or make someone else a leader before leaving.", Config.ERROR_COLOR, Config.ERROR_PREFIX));
            return false;
        }
