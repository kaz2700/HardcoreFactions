...
public class FLeaveArgument extends CommandArgument {
	...
	public boolean onCommand(CommandSender commandSender, Command command, String s, String[] strings) {
		if (!(commandSender instanceof Player player)) {
			commandSender.sendMessage(String.format("%s%sYou are the console so you can't leave a faction!",
		...
		if (!leavingPlayer.hasAFaction()) {
			player.sendMessage(String.format("%s%sYou must have a faction before leaving.", Config.ERROR_COLOR,
					Config.ERROR_PREFIX));
			return false;
		}
