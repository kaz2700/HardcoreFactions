...
public class FClaimArgument extends CommandArgument {
	...
	public boolean onCommand(CommandSender commandSender, Command command, String s, String[] strings) {
		...
		if (player.getInventory().contains(ClaimingWand.getClaimingWand())) {
			player.sendMessage(
					String.format("%s%sYou already have a claiming wand!", Config.ERROR_COLOR, Config.ERROR_PREFIX));
			return false;
		}

		if (!clearHotbar) {
			player.sendMessage(String.format("%s%sYou must have a clear slot in your hotbar!", Config.ERROR_COLOR,
					Config.ERROR_PREFIX));
			return false;
		}
