...
public class FClaimArgument extends CommandArgument {
  ...
  public boolean onCommand(
    ...
    if (faction == null) {
      player.sendMessage(
          String.format("%s%sYou don't have a faction!", Config.ERROR_COLOR, Config.ERROR_PREFIX));
      return false;
    }
    ...
    if (player.getInventory().contains(ClaimingWand.getClaimingWand())) {
      player.sendMessage(
          String.format(
              "%s%sYou already have a claiming wand!", Config.ERROR_COLOR, Config.ERROR_PREFIX));
      return false;
    }
    ...
    if (!clearHotbar) {
      player.sendMessage(
          String.format(
              "%s%sYou must have a clear slot in your hotbar!",
              Config.ERROR_COLOR, Config.ERROR_PREFIX));
      return false;
    }
    ...
    player.sendMessage(
        String.format("%sThe gods gifted you a claiming wand.", Config.SUCCESS_COLOR));
    return true;
  }
}
