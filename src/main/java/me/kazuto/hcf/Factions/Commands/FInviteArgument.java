...
public class FInviteArgument extends CommandArgument {
	...
	public boolean onCommand(CommandSender commandSender, Command command, String s, String[] strings) {
		...
		if (faction.getInvitedPlayers().contains(invitedFactionPlayer)) {
			player.sendMessage(String.format("%s%s%s is already invited to the faction.", Config.ERROR_COLOR,
					Config.ERROR_PREFIX, invitedFactionPlayer.getName()));
			return false;
		}

		if (faction.getPlayers().contains(invitedFactionPlayer)) {
			player.sendMessage(String.format("%s%s%s is already in the faction.", Config.ERROR_COLOR,
					Config.ERROR_PREFIX, invitedFactionPlayer.getName()));
			return false;
		}
