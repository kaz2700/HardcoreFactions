...
public class FInviteArgument extends CommandArgument {
  ...
  public boolean onCommand(
    ...
    if (FactionPlayerManager.getInstance().getPlayerFromName(strings[1]) == null) {
      player.sendMessage(
          String.format(
              "%s%sThat player never joined the server.", Config.ERROR_COLOR, Config.ERROR_PREFIX));
      return false;
    }

    FactionPlayer invitedFactionPlayer =
        FactionPlayerManager.getInstance().getPlayerFromName(strings[1]);
    ...
    if (faction.getInvitedPlayers().contains(invitedFactionPlayer)) {
      player.sendMessage(
          String.format(
              "%s%s%s is already invited to the faction.",
              Config.ERROR_COLOR, Config.ERROR_PREFIX, invitedFactionPlayer.getName()));
      return false;
    }
    ...
    if (faction.getPlayers().contains(invitedFactionPlayer)) {
      player.sendMessage(
          String.format(
              "%s%s%s is already in the faction.",
              Config.ERROR_COLOR, Config.ERROR_PREFIX, invitedFactionPlayer.getName()));
      return false;
    }

    if (invitedFactionPlayer.isOnline())
      invitedFactionPlayer
          .getOfflinePlayer()
          .getPlayer()
          .sendMessage(
              String.format(
                  "%s%s has invited you to join %s.",
                  Config.SUCCESS_COLOR, factionPlayer.getName(), faction.getName()));
    player.sendMessage(
        String.format(
            "%s You invited %s to the faction",
            Config.SUCCESS_COLOR, invitedFactionPlayer.getName()));
