  ...
  public boolean onCommand(
    ...
    if (!(commandSender instanceof Player player)) {
      commandSender.sendMessage(
          String.format(
              "%s%sYou need to be a player to execute this command.",
    ...
    if (strings[1].length() < 3 || strings[1].length() > 16) {
      player.sendMessage(
          String.format(
              "%s%sFaction name length must be between 3 and 16.",
              Config.ERROR_COLOR, Config.ERROR_PREFIX));
      return false;
    }
    ...
    if (factionPlayer.hasAFaction()) {
      player.sendMessage(
          String.format(
              "%s%sYou cannot be in a faction.", Config.ERROR_COLOR, Config.ERROR_PREFIX));
      return false;
    }
    ...
    PlayerFaction faction =
        (PlayerFaction) FactionManager.getInstance().createPlayerFaction(strings[1], factionPlayer);
