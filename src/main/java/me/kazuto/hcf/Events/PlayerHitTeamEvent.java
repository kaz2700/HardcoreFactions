...
public class PlayerHitTeamEvent implements Listener {
    ...
    public void playerHitTeam (EntityDamageByEntityEvent event) {
        ...
        PlayerFaction agressorFaction = FactionManager.getInstance().getFactionFromPlayer(agressorFactionPlayer);
        PlayerFaction victimFaction = FactionManager.getInstance().getFactionFromPlayer(victimFactionPlayer);
