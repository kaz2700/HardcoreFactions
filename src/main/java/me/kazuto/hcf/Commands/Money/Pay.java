...
public class Pay implements CommandExecutor {
    ...
    public boolean onCommand(@NotNull CommandSender commandSender, @NotNull Command command, @NotNull String label, @NotNull String[] strings) {
        ...
        FactionPlayer payedFactionPlayer = FactionPlayerManager.getInstance().getPlayerFromName(strings[0]);

        if (payedFactionPlayer == null) {
            player.sendMessage(String.format("%s%sPlayer not found!", Config.ERROR_COLOR, Config.ERROR_PREFIX));
            return false;
        }
        ...
        try {
            ...
            if (ammount <= 0) {
                player.sendMessage(String.format("%s%sAmmount must be positive!", Config.ERROR_COLOR, Config.ERROR_PREFIX));
                return false;
            }
        } catch (NumberFormatException exception) {
            player.sendMessage(String.format("%s%sAmmount must be a number!", Config.ERROR_COLOR, Config.ERROR_PREFIX));
            return false;
        }
